let stage, layer, nameText, signText, bgImageObj;
...
function initKonva(imageSrc) {
    ...
    bgImageObj.onload = function() {
        ...
        signText = new Konva.Text({
        text: 'SIGNATURE',
        x: width / 2,
        y: height * 0.8, // Move it lower, e.g., 80% down the canvas
        fontSize: Math.max(18, Math.floor(height / 16)),
        fontFamily: 'Arial',
        fill: 'red',
        draggable: true,
        ...
        signText.offsetX(nameText.width() / 2);
        signText.offsetY(nameText.height() / 2);
...
function createCertificatePreview(sampleName = "John Doe") {
    if (!nameText || !bgImageObj || !signText) {
    ...
    const previewText = new Konva.Text({
     ...
     const previewSign = new Konva.Text({
        text: signText.text(),
        x: signText.x() * scaleX,
        y: signText.y() * scaleY,
        fontSize: signText.fontSize() * ((scaleX + scaleY) / 2),
        fontFamily: signText.fontFamily(),
        fontStyle: signText.fontStyle(),
        fill: signText.fill(),
        shadowColor: signText.shadowColor(),
        shadowBlur: signText.shadowBlur(),
        shadowOffset: signText.shadowOffset(),
        shadowOpacity: signText.shadowOpacity(),
    });
    previewSign.offsetX(previewSign.width() / 2);
    previewSign.offsetY(previewSign.height() / 2);
    previewLayer.add(previewSign);
